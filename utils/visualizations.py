...
from assets.plotly_theme import (
    cadlabs_colors,
...
def fig_add_stage_markers(df, column, fig, secondary_y=None, parameters=parameters):
    ...
    for (name, date, (ay, ax)) in historical_dates:
        x_datetime = df.iloc[df.index.get_loc(date.strftime("%Y-%m-%d"), method="nearest")]["timestamp"]
        y_value = df.iloc[df.index.get_loc(date.strftime("%Y-%m-%d"), method="nearest")][column]
        fig.add_annotation(
            x=x_datetime,
            y=y_value,
            text=name,
    ...
    date_pos = parameters['date_pos'][0] 
    peak_eth_supply =  df.iloc[df.index.get_loc(date_pos.strftime("%Y-%m-%d"), method="nearest")]['eth_supply']
    if peak_eth_supply > df['eth_supply'].iloc[-1]:
        fig.add_annotation(x=date_pos, y=peak_eth_supply,
    ...
    for idx, (name, date) in enumerate(system_dates):
        nearest_row = df.index.get_loc(date.strftime("%Y-%m-%d"), method="nearest")
        fig.add_trace(
            go.Scatter(
                mode="markers+text",
                x=[date],
                y=[df.iloc[nearest_row][column]],
